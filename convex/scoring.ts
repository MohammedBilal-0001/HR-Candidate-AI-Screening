...
export function computeEligibility(assessments: any[]) {
  const mandatory = assessments.filter((a) => a.mandatory);
  if (mandatory.some((a) => a.result === "NO_MATCH" && a.confidence >= 0.75)) return { status: "INELIGIBLE" as const, reasons: ["A mandatory requirement was confidently not met."] };
  const uncertain = mandatory.filter((a) => a.result === "UNKNOWN" || a.result === "PARTIAL" || a.confidence < 0.75);
  if (uncertain.length) return { status: "REVIEW" as const, reasons: uncertain.map((a) => `"${a.requirementName}" is uncertain (${a.result}, ${Math.round(a.confidence * 100)}% confidence).`) };
  return { status: "ELIGIBLE" as const, reasons: [] };
}
export function scoreCandidate(assessments: any[], jd: any, weights: any, activeSkillNames: string[]) {
  ...
  const skillsScore = skills.reduce((sum: number, s: any) => {
    const a = assessments.find((x) => x.requirementName === s.name);
    return sum + (a ? values[a.result] : 0) * (totalImportance ? s.importance / totalImportance : 0);
  }, 0);
  const exp = assessments.find((a) => a.category === "experience");
  const candidateYears = exp?.candidateYears ?? 0;
  const requiredYears = Number(jd.experience?.minimumYears ?? 0);
  const experienceContribution = requiredYears > 0
    ...
    : 0;
  const edu = assessments.find((a) => a.category === "education");
  const other = assessments.filter((a) => a.category === "other");
...
export const recalculate = internalMutation({
  ...
  handler: async (ctx, { runId }) => {
    ...
    for (const member of members) {
      ...
      const eligibility = computeEligibility(assessments);
      const score = scoreCandidate(assessments, job.canonicalJson, run.weights, run.activeSkillNames);
